...
@implementation RubiconTests {
    ...
    -(NSString *)hexDebug:(NSUInteger)v {
        static dispatch_once_t     fhex = 0;
        static NSArray<NSString *> *hex = nil;
        ...
        [str appendFormat:@"0x%@", hex[q[0]]];
        for(int i = 1; i < 8; ++i) [str appendFormat:@" %@", hex[q[i]]];
        return str;
    }

    -(void)test79Endianness {
        uint8_t    data[]   = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16 };
        NSUInteger *vdata   = (NSUInteger *)data;
        NSUInteger oneWay   = 0x0102030405060708; // 0x01 02 03 04 05 06 07 08
        NSUInteger theOther = 0x0807060504030201; // 0x08 07 06 05 04 03 02 01

        NSLog(@"%@: %@", @"  One Way", [self hexDebug:oneWay]);
        NSLog(@"%@: %@", @"The Other", [self hexDebug:theOther]);
        NSLog(@"%@: %@", @"   Data 1", [self hexDebug:*vdata]);
        NSLog(@"%@: %@", @"   Data 2", [self hexDebug:*(vdata + 1)]);
